...
namespace engine
{
  ...
  void SwapChain::CreateSwapChain()
  {
    ...
    if (vkCreateSwapchainKHR(device.device(), &createInfo, nullptr, &swapChain) != 0)
      throw std::runtime_error("nie udalo sie stworzyc swap chain");
  ...
  void SwapChain::CreateRenderPass()
  {
    ...
    if (vkCreateRenderPass(device.device(), &renderPassInfo, nullptr, &renderPass) != 0)
  ...
  void SwapChain::CreateFramebuffers()
  {
    ...
    for (size_t i = 0; i < ImageCount(); i++)
    {
      ...
      if (vkCreateFramebuffer(device.device(), &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != 0)
  ...
  void SwapChain::CreateSyncObjects()
  {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
      if (vkCreateSemaphore(device.device(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != 0 ||
          vkCreateSemaphore(device.device(), &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != 0 ||
          vkCreateFence(device.device(), &fenceInfo, nullptr, &inFlightFences[i]) != 0)
  ...
  {
    // MOŻNA ZAKOMENDOWAĆ ŻEBY ZAWSZE BYŁ VSYNC
