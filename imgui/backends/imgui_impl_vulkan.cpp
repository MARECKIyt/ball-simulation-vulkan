...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    VkDescriptorSet last_desc_set = nullptr;
...
static VkPipeline ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, const ImGui_ImplVulkan_PipelineInfo* info)
{
    ...
    if (bd->VulkanInitInfo.UseDynamicRendering)
    {
        ...
        create_info.renderPass = nullptr; // Just make sure it's actually nullptr.
    }
#endif
    VkPipeline pipeline;
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    // Create pipeline
    bool create_main_pipeline = (v->PipelineInfoMain.RenderPass != nullptr);
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (bd->TexCommandBuffer)     { vkFreeCommandBuffers(v->Device, bd->TexCommandPool, 1, &bd->TexCommandBuffer); bd->TexCommandBuffer = nullptr; }
    if (bd->TexCommandPool)       { vkDestroyCommandPool(v->Device, bd->TexCommandPool, v->Allocator); bd->TexCommandPool = nullptr; }
    if (bd->TexSamplerLinear)     { vkDestroySampler(v->Device, bd->TexSamplerLinear, v->Allocator); bd->TexSamplerLinear = nullptr; }
    ...
    if (bd->DescriptorPool)       { vkDestroyDescriptorPool(v->Device, bd->DescriptorPool, v->Allocator); bd->DescriptorPool = nullptr; }
}
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info)
{
    ...
    if (info->DescriptorPool != nullptr) // Either DescriptorPool or DescriptorPoolSize must be set, not both!
        IM_ASSERT(info->DescriptorPoolSize == 0);
    else
        IM_ASSERT(info->DescriptorPoolSize > 0);
    if (info->UseDynamicRendering)
        IM_ASSERT(info->PipelineInfoMain.RenderPass == nullptr);
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count, VkImageUsageFlags image_usage)
{
    ...
    if (wd->UseDynamicRendering == false)
    {
        ...
        //ImGui_ImplVulkan_CreatePipeline(device, allocator, nullptr, wd->RenderPass, VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, v->Subpass);
    }
